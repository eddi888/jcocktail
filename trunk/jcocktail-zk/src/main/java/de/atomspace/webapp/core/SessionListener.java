...
public class SessionListener implements SessionCleanup, SessionInit{
	...
	public void init(Session session, Object request) throws Exception {
		log.info("init Session '"+session.getNativeSession().toString()+"'");
	}
	
	public void cleanup(Session session) throws Exception {
		log.info("cleanup Session '"+session.getNativeSession().toString()+"'");
	}
}
