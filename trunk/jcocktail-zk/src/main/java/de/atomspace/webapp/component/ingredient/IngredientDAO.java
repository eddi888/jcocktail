...
public class IngredientDAO  extends BasicDAO<Ingredient, String>{
	
	public IngredientDAO(Datastore ds) {
		super(ds);
	}
	
	public List<Ingredient> findAll(int offset, int limit) {
		Query<Ingredient> query = ds.createQuery(Ingredient.class);
		query = query.field("detached").equal(false);
		query = query.offset(offset);
		query = query.limit(limit);
		List<Ingredient> list = this.find(query).asList();
		return list;
	}
